...
- A weekly scheduled task ("Peterman Post weekly episode ingest", Mondays 7:06am PT) checks the archive API and fully ingests any new episode: transcript → source page → extract → entity/concept integration → `python3 data/build_graph_index.py` → `python3 data/rebuild_app.py` → SendUserFile + update_artifact (id: peterman-podcast-knowledge-graph). No-new-episode runs just append a `check` line to log.md.
